...
class FormatterGUI:
    def __init__(self, root):
        ...
        # Format Button
        self.run_button = tk.Button(root, text="Format Code", command=self.run_clipformatter)
    ...
    def run_clipformatter(self):
        exe_path = os.path.join(os.getcwd(), "ClipFormatter.exe")

        if not os.path.exists(exe_path):
            self.output_text.insert(tk.END, "ClipFormatter.exe not found in the current directory.\n")
            return
