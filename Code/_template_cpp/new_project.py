...
def copy_and_substitute(src: str, dst: str, project_name: str, project_snake: str) -> None:
    shutil.copytree(
        src, dst,
        ignore=shutil.ignore_patterns('new_project.py', '__pycache__', '*.pyc'),
    )

    # Rename files then dirs, bottom-up so parent renames don't invalidate child paths.
...
def main() -> None:
    ...
    print(f"  Build           : cmake -S . -B build && cmake --build build")
