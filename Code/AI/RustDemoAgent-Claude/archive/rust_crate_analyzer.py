...
class RustCrateAnalyzer:
    ...
    def generate_example(self) -> str:
        ...
        # Find constructors (new methods)
        constructors = [f for f in self.public_items['functions'] if f['name'] == 'new' and not f['is_method']]
        struct_with_new = {}
        for ctor in constructors:
            # Try to infer which struct this belongs to
            for struct in self.public_items['structs']:
                if struct['name'] in ctor['definition']:
                    struct_with_new[struct['name']] = struct
                    break
        
        # Generate struct examples
        if self.public_items['structs']:
            example_lines.append(f'    println!("\\n{section_num}. Creating instances...");')
            for struct in self.public_items['structs'][:3]:
                ...
                if struct_name in struct_with_new:
                    ...
                    example_lines.append(f'    println!("   Created {struct_name}");')
                else:
        ...
        if self.public_items['enums']:
            ...
            for enum in self.public_items['enums'][:3]:
                enum_name = enum['name']
                if enum.get('variants'):
                    first_variant = enum['variants'][0]
                    example_lines.append(f"    let _status = {enum_name}::{first_variant};")
                    example_lines.append(f'    println!("   Using {enum_name}::{first_variant}");')
                else:
                    example_lines.append(f"    // let _value = {enum_name}::Variant;")
            example_lines.append("")
            section_num += 1
        ...
        if self.public_items['constants']:
            ...
            for const in self.public_items['constants'][:3]:
