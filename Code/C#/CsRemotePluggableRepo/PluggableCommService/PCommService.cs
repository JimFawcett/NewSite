...
namespace PluggableRepository
{
  ...
  public class Receiver : IPluggableComm
  {
    ...
    {
      // address is like "http://localhost:8080/IPluggableComm"
      Uri uri = new Uri(address);
      string baseUrl = $"{uri.Scheme}://{uri.Host}:{uri.Port}";
      string path = uri.AbsolutePath; // "/IPluggableComm"

      var builder = WebApplication.CreateBuilder();
      builder.WebHost.UseUrls(baseUrl);
      builder.Services.AddServiceModelServices();

      commHost = builder.Build();
      commHost.UseServiceModel(sb =>
      {
        sb.AddService<Receiver>();
        sb.AddServiceEndpoint<Receiver, IPluggableComm>(
          new CoreWCF.BasicHttpBinding(), path);
      });

      commHost.StartAsync().GetAwaiter().GetResult();
    }
    ...
    public void close()
    {
      ...
      commHost?.StopAsync().GetAwaiter().GetResult();
    }
  ...
  public class Sender
  {
    ...
    public void createSendChannel(string address)
    {
      ...
      factory = new ChannelFactory<IPluggableComm>(binding, baseAddress);
