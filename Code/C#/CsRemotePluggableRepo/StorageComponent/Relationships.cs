...
namespace PluggableRepository
{
  ...
  public class Dependency
  {
    ...
    List<Package> descendents_ = new List<Package>();
    private HashSet<Package> visited_ = new HashSet<Package>();
    ...
    void dfsDescendents(Package pkg)
    {
      if (visited_.Contains(pkg))
        return;
      visited_.Add(pkg);
      descendents_.Add(pkg);
      foreach (Child child in getChildren(pkg))
        dfsDescendents(child);
    }
