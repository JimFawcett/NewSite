...
EntryPoint is a CLI tool that recursively searches a directory tree for files whose contents match a regular expression. It uses two local path dependencies (`rust_dir_nav` and `rust_cmd_line`) that must exist as sibling directories alongside this repo.
