...
- Constructs a `Cpp_TextFinder_Output` instance, passing formatting information from the parsed commands.
- Constructs a `Cpp_TextFinder_Dirnav` instance, templated on the `Cpp_TextFinder_Output` type, taking the `Cpp_TextFinder_Output` instance and the finalized skip list as constructor arguments.
- Owns the parsed `ProgramCommands` for the lifetime of the `Cpp_TextFinder_Dirnav` instance, which holds a reference to it rather than a copy.
...
2. If the parsed commands indicate `/H true`, write `Cpp_TextFinder_Cmdline::helpText()` to stdout, exit with code 0, and do not proceed.
3. If `/v true`, write `Cpp_TextFinder_Cmdline::optionsText(commands)` to stdout — one key/value pair per line — before traversal begins.
...
The binary implements `addSkipDirectory(name)` (Spec_TextFinder.md §3.5) as a code-level extension point — not exposed at runtime. Duplicate entries are ignored. The finalized list is a constructor argument to `Cpp_TextFinder_Dirnav`, which consults but never modifies it.

## 6. Multi-Path Handling

Per Spec_TextFinder.md §3.2 and §5, `/P` may appear multiple times and each occurrence adds a root path. `Cpp_TextFinder_Entry` preserves argv order when collecting root paths and drives traversal against them in that order using a single reused `Cpp_TextFinder_Dirnav` instance; the regex state machine compiled at Dirnav construction is reused across all roots. When no `/P` is supplied, the single default root path `.` is used.
