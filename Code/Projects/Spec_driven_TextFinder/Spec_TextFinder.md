...
Blocks and announcements are not the whole of what a traversing run writes. After the last root path is finished, TextFinder closes its output with the run summary of §3.6, which counts the files and directories the run reached and names none of them.
...
A run that traverses closes its output with a count of what it reached. After the last root path is finished and before the process exits, TextFinder writes one line to stdout, through the same output component the announcements of §3.4 go through:

    accessed <fileCount> files, <dirCount> directories

That line is the last thing stdout carries. The form is fixed here, since two implementations free to word it differently could not be compared line-for-line under §6. Both numbers are written in decimal with no grouping separator, and neither noun is inflected: a run that reached one file writes `1 files`. Inflecting would put a plural rule in four implementations to disagree over, and the cost of not inflecting — one line that reads badly in a case a real search rarely reaches — is accepted instead.

/h does not suppress the summary. It is neither a block nor an announcement: it names no path and reports no entry, so nothing in §3.4 governs it, and /h, which hides only files that matched nothing, leaves it in place. No switch suppresses it.

**What is counted.** A file counts when TextFinder examines it — when it passed the /p filter of §5 and TextFinder went on to take its size from the filesystem. Every file that reached selection and then produced a block, a file announcement, or an error announcement therefore counts, and so does every selected file in the no-content case of §3.3, though that case opens none of them; a selected file of zero size counts there too, though it produces neither a block nor an announcement. A root path that resolved to a regular file and was searched per §3.2 counts as a file.

Three things are not counted, each refused before selection. A file the /p list excluded is not counted, TextFinder having learned its name and nothing more; nor is a symbolic link, which §3.2 passes over without opening; nor is any file beneath a pruned directory, which traversal never reaches. Two entries that draw a `cannot open` announcement are not counted either, for the same reason: one whose name will not render, which §3.4 announces whatever /p holds, and one that is neither a regular file nor a directory. The count is therefore of files put to the tests of §3.3, not of announcements written.

A directory counts when TextFinder reads its entries or tries to — each root path that resolved to a directory, and each directory descended into per §3.2. A directory whose basename the skip list of §3.2 holds is not counted, pruning meaning it was never entered. A directory that drew a `cannot open` announcement is counted, because the attempt was made and these are counts of work attempted rather than of work that succeeded. Under /s `false` the root counts and nothing below it does.

The counts are of the whole run rather than of one root. They are written once, after the last root, and an entry reached under two /P roots counts once for each, since it was reached twice and each visit did its own work.

**Which component owns the counts.** This one internal division is fixed here rather than left to each component specification, so that all four implementations count in the same place and a count that disagrees is a defect rather than a difference of design. The traversal component holds the two counts as state of the value the entry binary starts at each root, and it increments them as it makes the selection decisions above — no other component sees the entries, so no other component can count them. The counts are never reset between roots, which is what makes them counts of the run. The traversal component also owns the summary's text, as it already owns the text of blocks and announcements, and writes it through the same output component. The entry binary decides when: after the last root is finished it calls the traversal component once, with no argument, to write the summary. Only the entry binary knows which root was the last, and this leaves it one call to make and the traversal component no notion of a run boundary to keep.

Each language-specific specification fixes the name, parameter list, and return type of that call and of any accessor it exposes, in the idiom of its own language, as §3.5 already does for `addSkipDirectory`. What it may not do is move the counting to another component.

**When it is written.** Every run that begins traversal writes the summary, and no other run does. The bare command line of §3.1 traverses nothing and writes the §5.3 listing alone; /H prints help and traverses nothing; a code-1 usage diagnostic and a code-2 startup failure both precede traversal. None of those four emits it. A run that begins traversal writes it whatever the traversal found, so a search that matched nothing still reports what it reached, and a run whose one root is a symbolic link writes `accessed 0 files, 0 directories` beneath that root's `cannot open` announcement.
...
**A runner builds what it is about to run.** It invokes the implementation's own build before the first suite, so that a fresh checkout needs no separate step and a reader cannot run yesterday's binary against today's source. A build that fails counts every suite the runner would have run as failed, and the runner stops there rather than running what happens to be on disk. The never-built rule above still applies after a build that succeeded: a suite whose executable is absent then is a failure, since the alternative is a runner that reports nothing and exits 0. Where a build needs environment a plain shell lacks, the runner establishes it or reports that it could not.

**A runner holds its console when it finishes.** A reader who starts one from a file manager sees its output rather than a window that closes, so the runner waits for a keystroke after its summary line. The wait comes last and does not change the exit code. It is suppressed when the environment variable `TEXTFINDER_NO_PAUSE` holds a non-empty value, so that a scripted run and the dated captures of §7.5 are not blocked; that name is fixed here rather than per implementation, so one setting covers all four runners.
